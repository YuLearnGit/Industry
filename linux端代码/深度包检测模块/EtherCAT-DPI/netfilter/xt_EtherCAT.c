...
static bool EtherCAT_mt4(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	const struct xt_EtherCAT_mtinfo *info = par->matchinfo;
...
/*
	//判断数据是客户端数据还是服务器端数据
	if(sport == htons(EtherCAT_PORT))
		data_src_flag = FROM_CLIENT_FLAG;
	else if(dport == htons(EtherCAT_PORT))
...
*/
	...
	if ( EtherCATDecode(data_start, app_len,FROM_CLIENT_FLAG, info,skb) == EtherCAT_FAIL)
	{
		#ifdef DEBUG
			printk("EtherCAT decode: not EtherCAT proto !!! Failed!!! \n");
			printk("================end==============\n");
		#endif
    ...
    {
		#ifdef DEBUG
    		printk("EtherCAT decode: EtherCAT proto !!! Success!!! \n");
    	    printk("================end==============\n");
		#endif
...
static struct xt_match EtherCAT_mt_reg __read_mostly = {
		.name      = "EtherCAT",
		.revision  = 1,
		.family    = NFPROTO_IPV4,
		.match     = EtherCAT_mt4,
		.matchsize = sizeof(struct xt_EtherCAT_mtinfo),
		.me        = THIS_MODULE,
};

static int __init EtherCAT_mt_init(void)
{
	#ifdef DEBUG
		printk("register match successed\n");
	#endif
	return xt_register_match(&EtherCAT_mt_reg);
}

static void __exit EtherCAT_mt_exit(void)
{
	#ifdef DEBUG
		printk("unregister match successed\n");
	#endif
	xt_unregister_match(&EtherCAT_mt_reg);
}

module_init(EtherCAT_mt_init);
module_exit(EtherCAT_mt_exit);
MODULE_LICENSE("GPL");
MODULE_AUTHOR("YRF");
MODULE_DESCRIPTION("iptables:EtherCAT detection");
MODULE_ALIAS("ipt_EtherCAT");
MODULE_ALIAS("ip6t_EtherCAT");
