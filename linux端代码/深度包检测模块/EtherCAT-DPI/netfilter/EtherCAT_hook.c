...
MODULE_DESCRIPTION("EtherCAT DPI");
...
static unsigned int EtherCAT_hook(unsigned int hooknum, struct sk_buff *skb, const struct net_device *in, const struct net_device *out, int (*okfn)(struct sk_buff *))
	...
	if(skb){
		...
		if(iph->protocol == IPPROTO_TCP){
           ...
           // dport=tcph->dest;
                ...
                printk("\n***************************\n");
                printk("\n*********EtherCAT dpi********\n");
                if ( EtherCATDecode(data_start, app_len) == EtherCAT_FAIL)
                	{
