...
MODULE_DESCRIPTION("Profinet DPI");
...
static unsigned int Profinet_hook(unsigned int hooknum, struct sk_buff *skb, const struct net_device *in, const struct net_device *out, int (*okfn)(struct sk_buff *))
	...
	if(skb){
		...
		if(iph->protocol == IPPROTO_TCP){
           ...
           // dport=tcph->dest;
                ...
                printk("\n***************************\n");
                printk("\n*********Profinet dpi********\n");
                if ( ProfinetDecode(data_start, app_len) == Profinet_FAIL)
                	{
