...
static unsigned int DNP3_hook(unsigned int hooknum, struct sk_buff *skb, const struct net_device *in, const struct net_device *out, int (*okfn)(struct sk_buff *))
           ...
           // dport=tcph->dest;
            ...
            if(tcph->dest == htons(20000)){
               ...
               // printk("Packet for source port: %d\nPacket for destination port: %d\n", htons(sport),htons(dport));
                ...
                printk("\n***************************\n");
                printk("\n*********DNP3 dpi********\n");
                if ( DNP3Decode(data_start, app_len) == DNP3_FAIL)
                	{
