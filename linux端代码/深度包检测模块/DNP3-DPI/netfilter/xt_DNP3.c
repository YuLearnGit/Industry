...
static bool DNP3_mt4(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	const struct xt_DNP3_mtinfo *info = par->matchinfo;
	...
	//判断数据是客户端数据还是服务器端数据
	if(sport == htons(DNP3_PORT))
		data_src_flag = FROM_CLIENT_FLAG;
	else if(dport == htons(DNP3_PORT))
	...
	if ( DNP3Decode(data_start, app_len, data_src_flag, info,skb) == DNP3_FAIL)
	{
		#ifdef DEBUG
			printk("DNP3 decode: not DNP3 proto !!! Failed!!! \n");
			printk("================end==============\n");
		#endif
    ...
    {
		#ifdef DEBUG
    		printk("DNP3 decode: DNP3 proto !!! Success!!! \n");
    	    printk("================end==============\n");
		#endif
...
static struct xt_match DNP3_mt_reg __read_mostly = {
		...
		.family    = NFPROTO_IPV4,
		.match     = DNP3_mt4,
		.matchsize = sizeof(struct xt_DNP3_mtinfo),
		.me        = THIS_MODULE,
};

static int __init DNP3_mt_init(void)
{
	#ifdef DEBUG
		printk("register match successed\n");
	#endif
	return xt_register_match(&DNP3_mt_reg);
}

static void __exit DNP3_mt_exit(void)
{
	#ifdef DEBUG
		printk("unregister match successed\n");
	#endif
	xt_unregister_match(&DNP3_mt_reg);
}
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("YRF");
MODULE_DESCRIPTION("iptables:DNP3 detection");
MODULE_ALIAS("ipt_DNP3");
MODULE_ALIAS("ip6t_DNP3");
