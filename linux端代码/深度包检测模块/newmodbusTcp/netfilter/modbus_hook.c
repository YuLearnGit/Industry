...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Wenjian Zhang");
MODULE_DESCRIPTION("Modbus DPI");
...
static unsigned int modbus_hook(unsigned int hooknum, struct sk_buff *skb, const struct net_device *in, const struct net_device *out, int (*okfn)(struct sk_buff *))
	...
	if(skb){
		...
		if(iph->protocol == IPPROTO_TCP){
            ...
            if(tcph->dest == htons(502)){
                ...
                printk("\n***************************\n");
                printk("\n*********modbus dpi********\n");
                if ( ModbusDecode(data_start, app_len) == MODBUS_FAIL)
                	{
